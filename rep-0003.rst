...
- C++03
  ...
  - Some C++11 will be enabled (TODO(esteve): need to determine which ones)
...
- Ogre3D 1.9.x
- Gazebo 5 (TODO(esteve): or Gazebo 6?)
- PCL 1.7.x (NOTE(esteve): maybe PCL 1.8 will be released by then, but Wily will only come with 1.7)
- OpenCV 2.4.x
...
TODO(esteve): clarify what C++11 feature might be exposed via the APIs
...
We will continue using C++03 features to preserve backwards compatibility, but we will enable certain features C++11 to provide a better experience for users and to integrate better with projects that are already using or planning to use C++11 (e.g. SDFormat).
